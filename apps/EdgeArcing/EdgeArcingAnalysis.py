...
for line in sys.stdin:
    # there is an additional comma in field:center
    lotid = line.split(',')[0].split('=')[1].strip("'")
    layer = line.split(',')[4].split('=')[1].strip("'")
    wafer_scribe = line.split(',')[7].split('=')[1].strip("'")
    designid = line.split(',')[2].split('=')[1].strip("'")
    waferid = line.split(',')[6].split('=')[1].strip("'")
    recipe = line.split(',')[11].split('=')[1].strip("'")
    slotid = line.split(',')[8].split('=')[1].strip("'")
    stepname = line.split(',')[3].split('=')[1].strip("'")
    imgname = line.split(',')[15].split('=')[1].strip("'")
    imgtext = line.split(',')[16].split('=')[1].strip("'") + '==='
    ...
    string_output = (lotid+","+layer+","+wafer_scribe+","+designid+","+waferid+","+
                     recipe+","+str(negative)+","+str(positive)+","+slotid+","+stepname)
    print(string_output)
